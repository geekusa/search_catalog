...
def dashboard_generate(
    ...
    pretty.add_base_search(
        search='|fieldsummary maxvals=0 |table field distinct_count values',
        search_id='sub_search',
    ...
    pretty.add_chart_panel(
        3,
        '| search NOT [| inputlookup most_populated_filter | format ] | sort -distinct_count | head 5 | table field distinct_count',
        title='Most Populated Fields',
        base_search='sub_search',
        height='137'
        )
    table_color_dict = {
        'count': 
	...
	pretty.add_single_value_panel(
            3,
            field_summary_search, 
            title='Number of fields/columns', 
            base_search='sub_search', 
            bgcolor='C3CEF4'
            )
        pretty.add_table_panel(
            4, 
            main_search, 
            title='Results', 
            search_id='base_search', 
            time_picker=True, 
            row_count='20'
            )
    else:	    
...
class DashboardGenerate(object):
    ...
    def add_row(self, num):
        """
	Add row(s) to the dashboard. 
        Provide a number for the quantity of rows added.
	"""
        i = 0
        while i < num: 
    ...
    def add_notes_panel(self, row_num, search, notes=False):
        """
	Generate a panel with no borders showing the search to be run along with 
        notes.
	"""
	...
	ET.SubElement(
            ...
            'a', 
            target='_blank', 
            href='/app/search/search?q='+urllib.quote_plus(search)+'&amp;display.page.search.tab=events&amp;display.general.type=events&amp;earliest=$time.earliest$&amp;latest=$time.latest$'
            ).text = 'Click to open in search'
    ...
    def add_chart_panel(
        ...
        """
	Generate a chart panel. Provide the row_number and search. 
	...
	panelchart = ET.SubElement(panel, 'chart')
        if base_search:
	    panelchartsearch = ET.SubElement(
                panelchart, 
                'search', 
                base=base_search
                )
	...
	panelsingle = ET.SubElement(panel, 'single')
        if base_search:
	    panelsinglesearch = ET.SubElement(
                panelsingle, 
                'search', 
                base=base_search
                )
	...
	paneltable = ET.SubElement(panel, 'table')
        if search_id:
            if base_search:
	        paneltablesearch = ET.SubElement(
                    paneltable, 
                    'search', 
                    base=base_search, 
                    id=search_id
	...
	if color_dict and isinstance(color_dict, dict):
		...
		paneltableformat = ET.SubElement(
                    paneltable, 
                    'format', 
                    field=color_field, 
                    type='color'
		...
		if value['colorPalette']['type'] == 'minMidMax':
		    paneltableformatcp = ET.SubElement(
                        paneltableformat, 
                        'colorPalette'
                        )
		    for k,v in value['colorPalette'].items():
		        paneltableformatcp.set(k, v)
		    paneltableformatscale = ET.SubElement(
                        paneltableformat, 
                        'scale'
                        )
		...
		if value['colorPalette']['type'] == 'map':
		    ET.SubElement(
                        ...
                        'colorPalette', 
                        type='map'
                        ).text = value['colorPalette']['_text']

    def add_events_panel(
                         self, 
                         row_num, 
                         search, 
                         title=False, 
